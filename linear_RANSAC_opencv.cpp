...
cv::Mat linear_RANSAC(const std::vector<cv::Point2f>& points, int iterations, double distance_threshold, int samples)
{
    // valiable for best model
    cv::Mat best_model;
    int best_inlier_count = 0;
    
    // random sampling
    std::mt19937 rng(std::random_device{}());
    for (int rept = 0; rept < iterations; rept++)
    {
        // select 2 random points
        std::uniform_int_distribution<int> dist(0, points.size() - 1);
        int idx1 = dist(rng); cv::Point2f point1 = points[idx1];
        int idx2 = dist(rng); cv::Point2f point2 = points[idx2];
        ...
        for (const auto& point : points)
        {
            ...
            if (distance < distance_threshold) inlier_count++;
        }
